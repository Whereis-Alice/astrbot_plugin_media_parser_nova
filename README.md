...
- YouTube 走官方 Innertube 接口多层降级解析，SABR 等疑难视频可选交给 yt-dlp 兜底，不依赖第三方镜像站
- 支持媒体缓存、媒体中转和 ZIP 归档
- 视频按聊天平台能收下的体积挑流，放不下就自动降档或改发封面，发送失败会在会话里说明原因
- 对下载体积、解析频率、缓存清理和公网 URL 做了限制与兜底
...
“下载设置”里有三个体积相关的配置，分别管**下载**、**发送**和**聚合**：

| 配置项 | 默认值 | 作用 |
| --- | --- | --- |
| 视频大小上限（`max_video_size_mb`） | 1000 MB | 下载上限。普通视频先预检 `Content-Length`，DASH/M3U8 按下载完成后的实际体积兜底；填 0 不限制 |
| 可发送视频体积上限（`send_video_max_mb`） | 100 MB | **聊天平台能收下的单条视频上限**。取流时优先挑放得下的画质，预估仍超限就跳过视频、只发信息与封面；填 0 不限制 |
| 大视频阈值（`large_video_threshold_mb`） | 100 MB | 超过阈值的视频单独发送，不放进合并转发；有效上限 100 MB，填 0 不限制 |

`send_video_max_mb` 单独存在的原因是：**下载得到 ≠ 发得出去**。QQ 的富媒体（Highway）通道通常在 100 MB 上下开始拒收，报 `[Highway] httpUpload Error ... code 102902`，而且常常是传到一半才拒——用户白等一两分钟，群里只看到一张卡片。现在插件会做三件事：

1. **取流时带体积预算**：YouTube 的 InnerTube 与 yt-dlp 两条链路都会先估算候选格式的体积，在预算内挑最高画质（例如 100 MB 预算下把 1080P 降到 720P），全部超限时退回最小的那一档。
2. **超限不再白下一遍**：预估或实际体积超过生效上限（两个上限取更小者）时直接跳过视频，改发「信息 + 封面」，并在消息里写清原因，例如「视频体积超过可发送上限（129.1MB > 100.0MB），聊天平台会拒收，已改为只发信息与封面」。
3. **发送失败会说话**：部分内容发送失败时，除了后台 WARNING，还会在会话里补一条 `⚠️ 有 1/2 项内容未能发出：视频上传被聊天平台拒收（体积过大或服务端限制）`，并附上原始链接方便直接点开。

不同协议端的实际上限差别不小，可按自己的部署实测值调整；把上限调大只影响插件是否愿意尝试，平台该拒还是会拒。
...
五套通用皮肤（极光 / 报章 / 测控 / 展陈 / 夜曲）的标题上方只保留「装饰记号 + 发布时间」这一条眉标：站点身份交给页脚的完整链接，内容是视频还是图文由媒体区块自己表达，不再往卡片顶部堆平台徽章和「视频 / 图文」标签。右侧仍会在有数据时补一枚「在线 N」。
...
X 的换件还多几处：时间行改成「时间 · N 查看」的原生语序（而不是把浏览量塞进页签），封面上不压播放量角标，每条回复正文下方会追加一行 回复 / 转推 / 喜欢 的迷你图标。
...
上面这些皮肤截图都可以本地复现，不依赖网络：`python tools/render_skin_docs.py`（只重出其中几张时把名称跟在后面，例如 `python tools/render_skin_docs.py x platform-chrome`）。
...
### yt-dlp（可选，只给 YouTube 疑难视频用）

刻意**没有**写进 `requirements.txt`：它只在 YouTube 的第 ④ 层兜底里用到，缺了就安静降级，不装也不影响其他任何平台。想让被 SABR 挡下的 YouTube 视频也能出视频文件，见 [疑难视频的 yt-dlp 兜底](#疑难视频的-yt-dlp-兜底)。
...
前三层是轻量 HTTP 请求，绝大多数视频到这里就结束了。第 ④ 层是给「Innertube 只下发 SABR 流」的疑难视频准备的后手，要拉起一个 JS 运行时、耗时数秒，所以只在前面全都失败时才付这份代价，详见 [疑难视频的 yt-dlp 兜底](#疑难视频的-yt-dlp-兜底)。
...
| 配置 | 默认 | 说明 |
| --- | --- | --- |
| 画质上限 | 1080 | 按视频高度限制。选“不限制”会尽量取最高画质；挑流时还会叠一层体积预算，放不下就自动降档，见 [视频体积与发送上限](#视频体积与发送上限) |
...
**InnerTube 与 yt-dlp 两条链路都会按体积挑流**：先估算每个候选格式的大小（`contentLength` / `filesize`，缺失时用 `tbr × 时长` 反推），在「可发送视频体积上限」之内取最高画质，全部超限时退回最小的那一档，避免下完 129 MB 才发现发不出去。
...
插件**自己不实现** PO token 与播放器 JS 签名还原：前者要跑一整套 BotGuard 虚拟机、后者随时被改，两者都会把这个解析器变成需要持续追着上游改的负担。这份维护成本转交给了 yt-dlp——需要执行播放器 JS 才能拿到的直链，由第 ④ 层兜底解决（见下）；连兜底也失败时，插件会退化成封面卡片而不是报错。
...
这种情况下插件会调用 **yt-dlp** 兜底。选择复用 yt-dlp 而不是自己写签名还原，理由很直接：上游每隔几周就换一次算法，追着改的成本远高于装一个包。
...
三件缺一不可：`yt-dlp` ≥ 2026.08.19 起把 JS 挑战外包给外部运行时，`yt-dlp-ejs` 提供求解脚本，运行时负责真正跑这段 JS。**只装 yt-dlp 是不够的**，这也是很多人只升级 yt-dlp 却依旧撞「Sign in to confirm you're not a bot」的原因。

> **一个很隐蔽的坑**：yt-dlp 的 `--js-runtimes` 默认值只有 `deno`，机器上装了 node 也不会被启用（日志里表现为 `JS runtimes: none`）。插件会自己探测并显式声明用哪个运行时，所以不需要你手动配；但如果你在命令行直接跑 yt-dlp 复现问题，记得加 `--js-runtimes node`。
...
- yt-dlp 早期那套 OAuth / 设备码登录方案**已经被 YouTube 封掉**，现在没有比 Cookie 更省事的合法登录途径。第 ④ 层的 yt-dlp 兜底同样要靠这份 Cookie 才能过门禁。
...
机器人验证、会员限定、地区限制、年龄限制、私享视频、正在直播等情况下拿不到可下载的流。**连 [yt-dlp 兜底](#疑难视频的-yt-dlp-兜底) 也失败时**插件不会报错，而是退化成「封面 + 标题 + 作者 + 时长 + 统计 + 热评」的卡片，并在卡片上标明原因（例如「被 YouTube 机器人验证挡下，仅展示封面与信息」）。
...
体积原因跳过视频时不算失败，也走同一套「封面 + 信息」降级，卡片和消息里会写明实际体积与上限，见 [视频体积与发送上限](#视频体积与发送上限)。

**取不到流时会额外打一条 WARNING**，一行写清全部上下文：尝试过的客户端链、当次的登录态（匿名 / Cookie 已鉴权 / Cookie 已失效）、代理是否启用、门禁返回的状态码与 `playabilityStatus`、yt-dlp 兜底链路的可用性，以及对应的处理建议（缺哪件就说装哪件）。此前这种情况只在群里显示一句「仅展示封面与信息」而后台一片安静，现在不会再出现「没解析出来但日志看不到任何线索」的情况。
