...
深色/浅色主题和布局选项只服务于 `Nova 原生` 皮肤。高级皮肤使用各自固定的配色与画面结构，不会因为“卡片主题”或“卡片布局”改变样式。封面全尺寸、播放按钮、自定义字体和署名仍可作用于支持它们的皮肤。
...
| 信号终端 | 绿青双色荧光信号台、作者身份面板、媒体监视窗、波形和透视网格 | 科技、游戏、数据感内容 |
| 海报档案 | 装裱海报、目录轨、裁切标、档案章、纸质说明卡和来源记录 | 视频封面、视觉作品、收藏归档 |
| 霓虹夜景 | 斜切媒体窗、纵向身份轨、青品红光带和透视夜景线 | 音乐、二次元、潮流与夜景内容 |
| B站动态 | 参考动态页的信息顺序，使用浅色作者头、正文优先、多图网格、互动栏和评论列表 | B站视频、动态、番剧与社交图文分享 |
...
| 霓虹夜景 | B站动态 |
| --- | --- |
| ![霓虹夜景皮肤](docs/card-skins/neon.png) | ![B站动态皮肤](docs/card-skins/bilibili.png) |

高级皮肤会忽略原生“布局”的画面结构，避免把同一张卡片简单套皮。卡片宽度、播放按钮、自定义字体和右下角署名仍然有效。

五套高级皮肤都会使用平台提供的作者头像；头像获取失败时才回退首字母占位。长作者名和作者 ID 会按各自作者区动态换行。

Twitter/X 普通推文没有独立标题时，插件不会再生成“作者名(@ID) 的推文”这类重复占位标题。普通文本直接展示作者、发布时间和正文；高级皮肤会把正文作为主内容绘制一次。信号终端遇到恰好两张图片的纯图片推文时，会使用两个完整媒体窗展示两张图。

多图内容不会再只把第一张图当作大图、把第二张图压成细条。两张图片会获得相近的展示面积；更多图片则按皮肤分别进入编辑室联系表、信号终端编号媒体窗、海报档案接触表、霓虹次级视觉窗或 B站动态网格。Nova 原生沉浸式布局也会在主视觉下方补充剩余图片；超出当前版式容量时，最后一个媒体窗会标出 `+N`。

卡片链接现在保留完整协议、路径、查询参数和片段，并按字符自动换行。Nova 原生四种布局和五套高级皮肤都不会再为了页脚宽度删除 `https://` 或直接截断链接。

信号终端不再使用 `//`、`LINK://` 等重复符号强调风格；海报档案也不再只是全幅封面，而是使用清晰可见的档案目录、装裱和来源记录结构。五套皮肤分别维护自己的版式、信息顺序和页脚，不是对同一布局替换颜色。
