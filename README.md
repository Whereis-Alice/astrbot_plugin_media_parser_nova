...
YouTube 的 Cookie 之所以「特别容易失效」，绝大多数不是过期，而是**被主动作废**：只要产出这份 Cookie 的浏览器会话还活着并继续刷新，或者你事后点了「登出」，服务端就会把对应的 `SAPISID` 作废。所以取 Cookie 的关键是让那个会话「就地冻结」。

推荐流程：

1. 开一个**无痕 / 隐私窗口**，登录准备当小号用的 Google 账号。
2. **在同一个标签页**跳转到 `https://www.youtube.com/robots.txt`。这一步是为了让页面停在一个不会自己发后台请求、也不会刷新令牌的静态页上。
...
- 论耐用度，**住宅代理往往比 Cookie 更划算**：代理不会「过期」，也不用定期回来重新导出，而机器人门禁本身很大程度上就是在看出口 IP 的信誉。条件允许时优先配 `代理设置 → YouTube`。
- Cookie 真的失效时插件不会闷着：日志出 WARNING，同时（默认开启）私聊管理员一条带上述 4 步指引的提醒，并带冷却避免刷屏。
