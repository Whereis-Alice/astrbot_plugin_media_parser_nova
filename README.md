...
- 支持卡片渲染、四种原生布局和三套独立高级皮肤
- 支持可选的大模型翻译，译文可只进入卡片或同时作为文本发送
...
| 布局 | 适用内容 | 视觉结构 |
| --- | --- | --- |
| 标准 | 通用视频、图文 | 顶部横幅加纵向信息区 |
| 杂志 | 标题和作者信息较多 | 左封面、右信息双栏 |
| 沉浸式 | 封面质量较高的视频 | 大幅封面加下方内容栈 |
| 信息流 | 社交动态、短正文 | 作者优先、媒体内嵌 |

深色/浅色主题、封面全尺寸和布局选项主要服务于 `Nova 原生` 皮肤。

### 独立高级皮肤

高级皮肤不是对原生卡片换色，而是使用各自独立的画布结构和信息层级：

| 皮肤 | 设计特点 | 推荐场景 |
| --- | --- | --- |
| 编辑室 | 纸张纹理、非对称双栏、编辑编号和缩略图索引 | 长标题、图文内容、作品分享 |
| 信号终端 | 网格底、媒体监视窗、遥测字段和状态栏 | 科技、游戏、数据感内容 |
| 海报档案 | 整幅封面、强标题层级、档案编号和底部信息带 | 视频封面、视觉作品、重点内容 |

| 编辑室 | 信号终端 | 海报档案 |
| --- | --- | --- |
| ![编辑室皮肤](docs/card-skins/editorial.png) | ![信号终端皮肤](docs/card-skins/signal.png) | ![海报档案皮肤](docs/card-skins/poster.png) |

高级皮肤会忽略原生“布局”的画面结构，避免把同一张卡片简单套皮。卡片宽度、播放按钮、自定义字体和右下角署名仍然有效。

当平台没有提供封面或封面下载失败时，插件使用仓库内置的原创无文字 Nova 抽象纹理，不会把固定角色或无关文案带进公开卡片。

### 自定义署名

“卡片右下角署名”默认为 `Nova解析`，可改成 `Alice解析` 等文字，最多 32 个字符。修改署名会自动使用新的卡片缓存键，不会复用旧署名图片。

## 翻译与卡片

翻译完成后，插件会把译文合并到一份卡片专用元数据副本中，因此卡片可以直接展示翻译后的标题和正文，而普通文本仍保留原始内容。

“翻译输出位置”有两种模式：

- **仅作用于卡片**：译文只用于卡片，不额外发送译文文本。需要同时开启卡片渲染才能看到译文。
- **卡片和文本都发送**：卡片使用译文，并在媒体解析结果后额外发送独立译文文本。
