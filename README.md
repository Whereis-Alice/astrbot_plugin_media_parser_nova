...
| 哔哩哔哩 | 还原 B 站视频卡：小电视标志、粉色品牌色、UP 主行与蓝色链接、图标化统计与评论列表 | B 站视频、动态、番剧 |
...
卡片最多展示前 5 条热评，并保留评论作者、ID、点赞数和时间。评论区会使用当前皮肤自己的视觉结构追加到同一张 PNG，不会把某套皮肤的评论面板套到另一套皮肤上。
...
当热评条数大于 `0` 且开启 Twitter/X 热评时，插件会尝试从无需登录的 X 公开帖子页提取当前访客可见的精选回复。这不是完整评论 API，因此有以下限制：
