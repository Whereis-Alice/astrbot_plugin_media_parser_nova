...
| Twitter/X | 视频、图片、文本、公开回复 | `twitter.com/.../status/...`、`x.com/.../status/...` |
...
当热评条数大于 `0` 且开启 Twitter/X 热评时，插件会尝试从无需登录的 X 公开帖子页提取当前访客可见的精选回复。提取按「页面内嵌 JSON-LD 结构化数据 → 内联 JS 兜底」的顺序进行，并同时尝试 `x.com` 与 `twitter.com` 两个域名；全部来源都失败时会在日志里逐条写明原因，而不是静默返回空列表。这不是完整评论 API，因此有以下限制：

- 只能获取页面当前公开展示的回复，无法保证覆盖完整讨论串
- 页面没有公开回复、地区或风控限制访问、或 X 调整页面结构时，会自动跳过回复，不影响正文解析
- 获取到的回复会按点赞数排序，并可与正文一起翻译、合并到卡片
